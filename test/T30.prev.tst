...
[0+2*x,1+2*y]: unknown, grown from [1]*4,16,1,1 -16*x^2+64*x^4-8*y-8*y^2=0 -> [2]
[1+2*x,1+2*y]: unknown 16*x+80*x^2+128*x^3+64*x^4-8*y-8*y^2=0 -> [3]
----------------
expanding queue[2]^1:  - 16*x^2 + 64*x^4 - 8*y - 8*y^2 = 0 meter=[1,2] *8
solution [0,1],trivial(1)
[0+2*x,1+4*y]: unknown, grown from [2]*1,1,2,4, grown from [1]*4,16,2,4 -16*x^2+64*x^4-16*y-32*y^2=0 -> [4]
...
[1+2*x,1+4*y]: unknown, grown from [3]*1,1,1,1,2,4 16*x+80*x^2+128*x^3+64*x^4-16*y-32*y^2=0 -> [6]
...
solution [0,1],trivial(1)
[0+4*x,1+8*y]: unknown, grown from [4]*4,16,2,4, grown from [2]*4,16,4,16, grown from [1]*16,256,4,16 -64*x^2+1024*x^4-32*y-128*y^2=0 -> [8]
...
solution [1,1],trivial(2)
[1+4*x,1+8*y]: unknown, grown from [6]*2,4,8,16,2,4, grown from [3]*2,4,8,16,4,16 32*x+320*x^2+1024*x^3+1024*x^4-32*y-128*y^2=0 -> [12]
...
solution [0,1],trivial(1)
[0+4*x,1+16*y]: unknown, grown from [8]*1,1,2,4, grown from [4]*4,16,4,16, grown from [2]*4,16,8,64, grown from [1]*16,256,8,64 -64*x^2+1024*x^4-64*y-512*y^2=0 -> [16]
expanding queue[9]^4: 448*x + 1472*x^2 + 2048*x^3 + 1024*x^4 - 160*y - 128*y^2 = 0 meter=[1,2] *32
solution [2,5],NONTRIVIAL
[2+4*x,5+16*y]: unknown, grown from [9]*1,1,1,1,2,4 448*x+1472*x^2+2048*x^3+1024*x^4-320*y-512*y^2=0 -> [17]
...
solution [1,1],trivial(2)
[1+8*x,1+16*y]: unknown, grown from [12]*2,4,8,16,2,4, grown from [6]*4,16,64,256,4,16, grown from [3]*4,16,64,256,8,64 64*x+1280*x^2+8192*x^3+16384*x^4-64*y-512*y^2=0 -> [20]
