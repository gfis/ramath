...
# @(#) $Id: parm3for.pl 221 2009-08-11 06:08:05Z gfis $
...
use strict;
	...
	foreach (0, -1, 1, -2, 2) { #; while ($w[4] != $limit) {
		$w[4] = $_;
	foreach (0, -1, 1, -2, 2) { # $x[4] = 0; while ($x[4] != $limit) {
		$x[4] = $_;
	if ($w[4] ge $x[4]) {
	foreach (0, -1, 1, -2, 2) { # $y[4] = 0; while ($y[4] != $limit) {
		$y[4] = $_;
	if ($x[4] ge $y[4]) {
		$coeff12 = 0
			 +   $w[4]*$w[4]*$w[4]
		...
		if (defined($root{$coeff12})) {
			$z[4] = $root{$coeff12};
			$d3z4z4 = 3*$z[4]*$z[4];
			foreach (0, -1, 1, -2, 2) { # $w[0] = 0; while ($w[0] != $limit) {
				$w[0] = $_;
			foreach (0, -1, 1, -2, 2) { # $x[0] = 0; while ($x[0] != $limit) {
				$x[0] = $_;
			foreach (0, -1, 1, -2, 2) { # $y[0] = 0; while ($y[0] != $limit) {
				$y[0] = $_;
				...
				if (defined($root{$coeff00})) {
					$z[0] = $root{$coeff00};
					$d3z0z0 = 3*$z[0]*$z[0];
					foreach (0, -1, 1, -2, 2) { # $w[3] = 0; while ($w[3] != $limit) {
						$w[3] = $_;
					foreach (0, -1, 1, -2, 2) { # $x[3] = 0; while ($x[3] != $limit) {
						$x[3] = $_;
					if ("$w[4]$w[3]" ge "$x[4]$x[3]") {
					foreach (0, -1, 1, -2, 2) { # $y[3] = 0; while ($y[3] != $limit) {
						$y[3] = $_;
					if ("$x[4]$x[3]" ge "$y[4]$y[3]") {
						...
						if ($z[4] == 0 || $coeff11 % $d3z4z4 == 0) {
							foreach (0, -1, 1, -2, 2) { # $w[1] = 0; while ($w[1] != $limit) {
								$w[1] = $_;
							foreach (0, -1, 1, -2, 2) { # $x[1] = 0; while ($x[1] != $limit) {
								$x[1] = $_;
							foreach (0, -1, 1, -2, 2) { # $y[1] = 0; while ($y[1] != $limit) {
								$y[1] = $_;
								...
								if ($z[0] == 0 || $coeff01 % $d3z0z0 == 0) {
										foreach (0, -1, 1, -2, 2) { # $w[2] = 0; while ($w[2] != $limit) {
											$w[2] = $_;
										$vw = "$w[4]$w[3]$w[2]$w[1]$w[0]";
										foreach (0, -1, 1, -2, 2) { # $x[2] = 0; while ($x[2] != $limit) {
											$x[2] = $_;
										$vx = "$x[4]$x[3]$x[2]$x[1]$x[0]";
										if ($vw ge $vx) {
										foreach (0, -1, 1, -2, 2) { # $y[2] = 0; while ($y[2] != $limit) {
											$y[2] = $_;
										$vy = "$y[4]$y[3]$y[2]$y[1]$y[0]";
										if ($vx ge $vy) {
											&test();
...
...
	if (1) { 
		...
		|| $coeff11 != 3*$z[4]*$z[4]*$z[3]
		|| $coeff10 != 3*$z[4]*$z[4]*$z[2] + 3*$z[4]*$z[3]*$z[3]
		|| $coeff09 !=   $z[3]*$z[3]*$z[3] + 3*$z[4]*$z[4]*$z[1] + 3*$z[4]*$z[3]*$z[2]
		|| $coeff08 != 3*$z[4]*$z[4]*$z[0] + 3*$z[4]*$z[3]*$z[1] + 3*$z[4]*$z[2]*$z[2] + 3*$z[3]*$z[3]*$z[2]
