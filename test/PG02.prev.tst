make[1]: Entering directory '/cygdrive/c/Users/gfis/work/gits/ramath'
cat test/progen.c
/* Do   N O T   edit here! */
/* Generated by org.teherba.ramath.linear.ProgramGenerator on yyyy-mm-dd hh:mm */
#include <stdio.h>
int main(int argc, char *argv[]) {
int m11,m12,m13
,m21,m22,m23
,m31,m32,m33
;
int p = 3; int q = 4; int r = 5; 
/* mono2= + a*b, rset1=
[0]  + a^4*(m11^2 + 2*m11*m13 + m13^2 + m21^2 + 2*m21*m23 + m23^2 - m31^2 - 2*m31*m33 - m33^2)
[1]  + 4*a^3*b*(m11*m12 + m12*m13 + m21*m22 + m22*m23 - m31*m32 - m32*m33)
[2]  + 2*a^2*b^2*( - m11^2 + 2*m12^2 + m13^2 - m21^2 + 2*m22^2 + m23^2 + m31^2 - 2*m32^2 - m33^2)
[3]  + 4*a*b^3*( - m11*m12 + m12*m13 - m21*m22 + m22*m23 + m31*m32 - m32*m33)
[4]  + b^4*(m11^2 - 2*m11*m13 + m13^2 + m21^2 - 2*m21*m23 + m23^2 - m31^2 + 2*m31*m33 - m33^2) */
for (m11 = -3; m11 < 4; m11++) {
if (m11 != 0) {
for (m12 = -3; m12 < 4; m12++) {
if (m12 != 0) {
for (m13 = -3; m13 < 4; m13++) {
if (m13 != 0) {
int p1 = m11*p + m12*q + m13*r; 
if (p1 > 0) {
for (m21 = -3; m21 < 4; m21++) {
if (m21 != 0) {
for (m22 = -3; m22 < 4; m22++) {
if (m22 != 0) {
for (m23 = -3; m23 < 4; m23++) {
if (m23 != 0) {
if (m21 != m11 || m22 != m12 || m23 != m13) {
int q1 = m21*p + m22*q + m23*r; 
if (q1 > 0) {
for (m31 = -3; m31 < 4; m31++) {
if (m31 != 0) {
for (m32 = -3; m32 < 4; m32++) {
if (m32 != 0) {
for (m33 = -3; m33 < 4; m33++) {
if (m33 != 0) {
if (m31 != m21 || m32 != m22 || m33 != m23) {
if (m31 != m11 || m32 != m12 || m33 != m13) {
int r1 = m31*p + m32*q + m33*r; 
if (r1 > 0) {
if (m11 != m12 || m21 != m22 || m31 != m32) {
if (m11 != m13 || m21 != m23 || m31 != m33) {
if (m12 != m13 || m22 != m23 || m32 != m33) {
if (m11^2 + 2*m11*m13 + m13^2 + m21^2 + 2*m21*m23 + m23^2 - m31^2 - 2*m31*m33 - m33^2 == 0) /*  + a^4 */ {
if (m11*m12 + m12*m13 + m21*m22 + m22*m23 - m31*m32 - m32*m33 == 0) /*  + 4*a^3*b */ {
if ( - m11^2 + 2*m12^2 + m13^2 - m21^2 + 2*m22^2 + m23^2 + m31^2 - 2*m32^2 - m33^2 == 0) /*  + 2*a^2*b^2 */ {
if ( - m11*m12 + m12*m13 - m21*m22 + m22*m23 + m31*m32 - m32*m33 == 0) /*  + 4*a*b^3 */ {
if (m11^2 - 2*m11*m13 + m13^2 + m21^2 - 2*m21*m23 + m23^2 - m31^2 + 2*m31*m33 - m33^2 == 0) /*  + b^4 */ {
if (p1*p1+q1*q1-r1*r1 == 0) {
printf("[");
printf("[%d,%d,%d]",m11,m12,m13);
printf(",");
printf("[%d,%d,%d]",m21,m22,m23);
printf(",");
printf("[%d,%d,%d]",m31,m32,m33);
printf("]");
printf("	preserves [%d,%d,%d]\n",p1,q1,r1);
}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}}
} /* main */
gcc -o test/progen.exe test/progen.c
test/progen.exe
[[-3,1,2],[1,1,1],[-2,1,3]]	preserves [5,12,13]
[[-2,1,1],[2,2,-2],[-2,-1,3]]	preserves [3,4,5]
[[-2,1,2],[-1,1,1],[-3,1,3]]	preserves [8,6,10]
[[-2,1,2],[-1,2,2],[-2,2,3]]	preserves [8,15,17]
[[-2,2,2],[1,2,1],[-1,2,3]]	preserves [12,16,20]
[[-2,2,2],[2,1,-1],[-2,1,3]]	preserves [12,5,13]
[[-1,1,1],[-2,1,2],[-3,1,3]]	preserves [6,8,10]
[[-1,2,1],[2,2,2],[1,2,3]]	preserves [10,24,26]
[[-1,2,2],[-2,1,2],[-2,2,3]]	preserves [15,8,17]
[[1,-2,2],[2,-1,2],[2,-2,3]]	preserves [5,12,13]
[[1,-1,1],[3,1,-2],[-2,-1,3]]	preserves [4,3,5]
[[1,1,1],[-3,1,2],[-2,1,3]]	preserves [12,5,13]
[[1,2,-1],[2,-2,2],[1,-2,3]]	preserves [6,8,10]
[[1,2,1],[-2,2,2],[-1,2,3]]	preserves [16,12,20]
[[1,2,2],[2,1,2],[2,2,3]]	preserves [21,20,29]
[[2,-2,2],[1,2,-1],[1,-2,3]]	preserves [8,6,10]
[[2,-2,2],[2,1,1],[2,-1,3]]	preserves [8,15,17]
[[2,-1,1],[2,2,2],[2,1,3]]	preserves [7,24,25]
[[2,-1,2],[1,-2,2],[2,-2,3]]	preserves [12,5,13]
[[2,1,-1],[-2,2,2],[-2,1,3]]	preserves [5,12,13]
[[2,1,1],[2,-2,2],[2,-1,3]]	preserves [15,8,17]
[[2,1,2],[1,2,2],[2,2,3]]	preserves [20,21,29]
[[2,2,2],[-1,2,1],[1,2,3]]	preserves [24,10,26]
[[2,2,2],[2,-1,1],[2,1,3]]	preserves [24,7,25]
make[1]: Leaving directory '/cygdrive/c/Users/gfis/work/gits/ramath'
