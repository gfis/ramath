...
public class MatrixGenerator {
    public final static String CVSID = "@(#) $Id: MatrixGenerator.java 808 2011-09-20 16:56:14Z gfis $";
        ...
        switch (numVars) {
            case 1: // univariate: (h3*x^3 + h2*x^2 + h1*x + h0)^3 - (a3*x^3 + a2*x^2 + a1*x + a0)^3 - ...
                while (dispenser.hasNext()) {
                    ...
                    int temp = 0;
                    int factor = Vector.extractGcd(sums);
                    int imax = 0;
        ...
        while (busy && iarg < args.length) {
            ...
            if (opt.matches("\\-[a-z].*")) {
                if (false) {
                    ...
                    System.err.println("**MatrixGenerator.process1: invalid option \"" + opt + "\"");
                }
            } else {
                busy = false;
            }
        ...
        while (busy && iarg < args.length) {
            ...
            if (opt.matches("\\-\\w.*")) {
                if (false) {
                    ...
                    System.out.println("gcd(" + limit + ", " + factor + ") = " + Vector.gcd(limit, factor));
                    System.exit(0);
